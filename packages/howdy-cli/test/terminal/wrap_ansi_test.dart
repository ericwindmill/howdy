...
void main() {
  group('String.wrapAnsi', () {
    test('does not modify string if width is infinite or <= 0', () {
      expect('hello world'.wrapAnsi(0), 'hello world');
      expect('hello world'.wrapAnsi(-1), 'hello world');
    });

    test('does not wrap if text is shorter than width', () {
      expect('hello world'.wrapAnsi(20), 'hello world');
    });

    test('wraps on space within width', () {
      // "hello world" is 11 chars.
      // Wrapping at 5 should split "hello" and "world"
      expect('hello world'.wrapAnsi(5), 'hello\nworld');
    });

    test('wraps multiple times', () {
      expect('the quick brown fox'.wrapAnsi(5), 'the\nquick\nbrown\nfox');
    });

    test('preserves existing newlines and resets width', () {
      expect('hello\nworld'.wrapAnsi(5), 'hello\nworld');
      expect(
        'a very long\nstring that'.wrapAnsi(10),
        'a very\nlong\nstring\nthat',
      );
    });
    ...
    test('hard wraps words longer than max width', () {
      ...
      expect('supercalifragilistic'.wrapAnsi(5), 'super\ncalif\nragil\nistic');
    });
